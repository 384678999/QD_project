                ...
                <el-col :span="6">
                    <el-form ref="formData" :model="formData" :rules="rules" label-width="100px">
                        <el-form-item label="旧密码" prop="old_pwd">
                            <el-input v-model="formData.old_pwd"  :show-password="true"></el-input>
                        </el-form-item>
        ...
        methods:{
            onSubmit(formData) {
                this.$refs[formData].validate((valid) => {
                    if (valid) {
                        if(this.formData.old_pwd == this.formData.password){
                            this.$notify.error({title: '错误', message: '输入不一样的密码'});
                            return false;
                        }
                        if(this.formData.password != this.formData.for_pwd){
                            this.$notify.error({title: '错误', message: '两次新密码不一致'});
                            return false;
                        }
                        this.formData.id = this.$cookies.get('user_id');
                        this.$api.fun.usersPwd(this.formData).then((res)=>{
                            if(res.status){
